...
class DANLivePX4Executor(Node):
    ...
    def control_tick(self) -> None:
        ...
        # Moving mission: start DAN routing only after all drones completed sequential takeoff.
        if (not self.args.hover_only) and (not getattr(self, "_routes_started", False)):
            if all(drone.phase == "HOVER" for drone in self.drones):
        ...
        for drone in self.drones:
            ...
            if drone.phase == "WARMUP":
                ...
                interval_done = now >= self.next_takeoff_allowed_s

                if warmup_done and is_next_drone and interval_done:
                    self.get_logger().info(
                        f"Sequential takeoff: starting Drone {drone.index + 1}"
                    )
            ...
            if drone.phase == "TAKEOFF":
                ...
                # PX4 can miss/reject the first command. Retry while taking off.
                ...
                if self.dwell_satisfied(drone, reached):
                    ...
                    if drone.index == self.next_takeoff_index:
                        ...
                        if self.next_takeoff_index < self.num_agents:
                            self.get_logger().info(
                                f"Sequential takeoff: Drone {drone.index + 1} is HOVER. "
                                f"Drone {self.next_takeoff_index + 1} may start in "
                                f"{self.takeoff_interval_s:.1f} s."
                            )
                        else:
                            self.get_logger().info(
                                "Sequential takeoff: all drones have reached HOVER."
                            )
            ...
            if drone.phase == "ROUTE":
                if drone.current_task_id is None:
                    ...
                    if not assigned:
                        # No remaining tasks: send this drone back to its home/depot.
                        # Some patched executor versions lost self.depot_unit, so recover it from the mission.
